...
AuraHub lets you save and restore your entire UI setup — WeakAuras, ElvUI profile, Skada layout, DBM settings, keybinds, macros, and action bar layouts — with a single click. It also ships a sync script that copies your backups across multiple WoW accounts automatically.
...
| **DBM** | All Deadly Boss Mods settings including per-boss checkboxes |
...
- **DBM** (Deadly Boss Mods) addon — only needed for the DBM tab
...
1. Configure Deadly Boss Mods to the desired state (options, per-boss checkboxes, etc.).
2. Go to the **DBM** tab → click **+ Save DBM Profile**.
3. The profile name is taken from your active DBM profile (set inside DBM settings).
4. To restore: click **Restore** → confirm → **Reload Now**.

> **Per-boss checkboxes** (e.g. ICC announce settings) are stored in LoadOnDemand addon variables. They are applied correctly when you enter the corresponding instance after restoring — this is expected WoW behavior, not a bug.
...
Open File Explorer, navigate to your **WoW root folder** (where `Wow.exe` is), and double-click **`SyncAuraHub.bat`**.
...
- Save an external backup to `Documents\AuraHubBackup\`.

**Step 4 — Log into any account**

All your WeakAuras, ElvUI, Skada, DBM, Keybinds, Macros, and Bar layouts are now available on every account.

For **DBM profiles**: after logging in, open AuraHub → DBM tab → click **Restore** on the desired profile → reload.
...
| ElvUI, Skada, DBM, Keybinds, Macros, Bars | `name` | Newer `savedAt` wins |
