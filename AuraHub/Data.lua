...
local function SyncOne(name)
    ...
    local merged = 0
    for _, key in ipairs({ "WeakAuras", "ElvUI", "Skada", "DBM", "Binds", "Macros", "ActionBars" }) do
...
function D.GetWAs()    return SortDesc(AuraHubDB.WeakAuras) end
...
function D.DeleteWA(index)
    local e = AuraHubDB.WeakAuras[index]
    if e then Tombstone("WeakAuras", e.name) end
...
function D.GetElvUIs()   return SortDesc(AuraHubDB.ElvUI) end
...
function D.GetSkadas()   return SortDesc(AuraHubDB.Skada) end
...
-- File I/O public helpers

function D.ReadFile(path)  return TryRead(path)  end
function D.WriteFile(path, content) return TryWrite(path, content) end

-- DBM

function D.GetDBMs()   return SortDesc(AuraHubDB.DBM) end
function D.CountDBMs() return #AuraHubDB.DBM end

function D.SaveDBM(entry)
    ClearTombstone("DBM", entry.name)
    table.insert(AuraHubDB.DBM, entry)
    WriteShared()
end

function D.DeleteDBM(index)
    local e = AuraHubDB.DBM[index]
    if e then Tombstone("DBM", e.name) end
    table.remove(AuraHubDB.DBM, index)
    WriteShared()
end

-- Binds

function D.GetBinds()   return SortDesc(AuraHubDB.Binds) end
...
function D.GetMacros()   return SortDesc(AuraHubDB.Macros) end
...
function D.GetActionBars()   return SortDesc(AuraHubDB.ActionBars) end
