...
local function IsEntryLoaded(entry)
    if not AuraHub.WA.IsReady() then return false end
    if not entry.displays or #entry.displays == 0 then return false end
    local db = AuraHub.WA.GetDisplaysTable()
    if not db then return false end
    local rootId = entry.displays[1] and entry.displays[1].id
    return rootId ~= nil and db[rootId] ~= nil
end
...
function P.Refresh()
    ...
    for i, entry in ipairs(entries) do
        ...
        local loaded = IsEntryLoaded(entry)
        ...
        local bgCol, borderCol
        if loaded then
            bgCol     = { 0.07, 0.17, 0.09, 1 }
            borderCol = { 0.22, 0.55, 0.28, 1 }
        else
            bgCol     = (visIdx % 2 == 0) and C.card or C.cardAlt
            borderCol = C.border
        end
        UI.AB(card, bgCol, borderCol)
        ...
        nameFs:SetTextColor(loaded and { 0.55, 0.90, 0.60 } or C.text)
        nameFs:SetTextColor(
            loaded and 0.55 or C.text[1],
            loaded and 0.90 or C.text[2],
            loaded and 0.60 or C.text[3]
        )
        ...
        if loaded then
            -- show "Active" badge instead of Restore button
            local activeFs = card:CreateFontString(nil, "OVERLAY")
            activeFs:SetPoint("RIGHT", card, "RIGHT", -80, 0)
            UI.SetFont(activeFs, 13)
            activeFs:SetText("|cff44cc55" .. L["WA_BADGE_ACTIVE"] .. "|r")
            queueBtn:Hide()
        else
        ...
        local function SetCardVisual(hovering)
            if loaded then return end  -- loaded cards keep their green style always
            if sel then
            ...
            elseif hovering then
                ...
                card:SetBackdropBorderColor(borderCol[1], borderCol[2], borderCol[3], borderCol[4] or 1)
        ...
        selZone:SetScript("OnEnter", function() if not loaded then SetCardVisual(true)  end end)
        selZone:SetScript("OnLeave", function() if not loaded then SetCardVisual(false) end end)
