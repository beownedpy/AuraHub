...
function P.Refresh()
    ...
    for i, entry in ipairs(entries) do
        ...
        local formTag = ""
        if (entry.form or 0) > 0 then
            local hasStanceSlots = false
            if entry.data then
                for _, s in ipairs(entry.data) do
                    if s.slot >= 73 then hasStanceSlots = true; break end
                end
            end
            if hasStanceSlots then
                local label = (entry.formName and entry.formName ~= "") and entry.formName or ("Form " .. entry.form)
                formTag = "  |cff88aaff[" .. label .. "]|r"
            end
        end
