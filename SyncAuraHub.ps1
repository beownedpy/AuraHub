...
$sections = @("WeakAuras", "ElvUI", "Skada", "DBM", "Binds", "Macros", "ActionBars")
...
# Save merged result to Documents so it survives WTF deletion or reinstall
if ($ok -gt 0) {
    $backupDir = Join-Path $env:USERPROFILE "Documents\AuraHubBackup"
    if (-not (Test-Path $backupDir)) { New-Item -ItemType Directory -Path $backupDir | Out-Null }
    $timestamp  = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
    $backupFile = Join-Path $backupDir "AuraHub_$timestamp.lua"
    [System.IO.File]::WriteAllText($backupFile, $result, $enc)
    Write-Host "  External backup: $backupFile" -ForegroundColor DarkGray
